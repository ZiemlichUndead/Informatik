...
(define (ableiten ausdruck var)
  (cond ((number? ausdruck) 0)
        ((variable? ausdruck)
         (if (gleiche-variable? ausdruck var) 1 0))
        ((summe? ausdruck)
         (konstr-summe (ableiten (addend ausdruck) var)
                       (ableiten (augend ausdruck) var)))
        ((produkt? ausdruck)
         (konstr-summe
          (konstr-produkt (multiplikator ausdruck)
                          (ableiten (multiplikand ausdruck) var))
          (konstr-produkt (ableiten (multiplikator ausdruck) var)
                          (multiplikand ausdruck))))
        (else
         (display "Unbekannter Ausdruck in ableiten" ausdruck))))